...
class FantaMKGitHubSource<T>(
    ...
    override suspend fun getLatestInfo() = withContext(Dispatchers.IO) {
        ...
        require(candidates.size == 1) {
            "Expected exactly one Nexora .rvp asset, found ${candidates.size}"
        }

        val asset = candidates.single()
        val publishedAt = release.publishedAt
            ?: error("Nexora GitHub release has no published_at timestamp")
    ...
    companion object {
        ...
        const val DISPLAY_NAME = "Nexora Patches"
        private const val USER_AGENT = "Nexora-Manager"
